import { env, getDestinationWidthBySize, getValueOrCallback, } from '@responsive-image/core';
...
function getLayout(props) {
    const layout = props.width === undefined && props.height === undefined
        ? 'responsive'
        : 'fixed';
    return layout;
}
...
export function ResponsiveImage(props) {
    ...
    const { src, size, sizes, width, height, className, ...htmlAttributes } = props;
    const riProps = {
        src,
        size,
        sizes,
    ...
    const img = (React.createElement("img", { key: key, className: getClassNames(riProps, isLoaded, className), loading: htmlAttributes.loading || 'lazy', decoding: htmlAttributes.decoding || 'async', width: getWidth(riProps), height: getHeight(riProps), srcSet: src.imageTypes === 'auto'
            ? // auto format assumes only one entry in sources
                sources[0]?.srcset
            : undefined, src: getSrc(riProps), ...htmlAttributes, "data-ri-lqip": riProps.src.lqip?.attribute, style: getStyles(riProps, isLoaded), onLoad: () => setLoaded(src) }));
    if (src.imageTypes === 'auto') {
        return img;
    }
    return (React.createElement("picture", null,
        sources
            .sort((a, b) => (typeScore.get(b.type) ?? 0) - (typeScore.get(a.type) ?? 0))
            .map((s) => (React.createElement("source", { key: s.mimeType, srcSet: s.srcset, type: s.mimeType, sizes: s.sizes }))),
        img));
}
